    ...
    def average_over_families(self, input_dict, remove_empty = False):
        ...
        try:
            ...
            for i in range(len(self.families)):
                ...
                if family_values:
                    output_dict[family_name] = statistics.mean(family_values)
                else:
                    if not remove_empty:
            ...
            if i in exons.keys():
                ...
                if CDS[i]:
                    if CDS[i][0][6] == "+":
                        ...
                        shift = [uf_records[j][2] + remainder[j] if remainder[j]%3 == 0 else uf_records[j][2] + remainder[j]//3 + 3 for j in range(len(CDS[i]))]  
                    else:
                        ...
                        shift = [df_records[j][2] + remainder[j] if remainder[j]%3 == 0 else df_records[j][2] + remainder[j]//3 for j in range(len(CDS[i]))]
    ...
    def get_intron_densities(self, exons, CDS):
        ...
        for i in self.names:
            if not exons[i] or not CDS[i]:
                ...
                intron_densities[i] = current_exon_number/current_CDS_length
